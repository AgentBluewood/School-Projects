...
require_once('db_conn.php');  
    ...
    if (isset($_SESSION['id'])) { //Check if session ID is set and if user is successfully logged in before allowing to edit profile
        ...
        $updatedItems = "";
        $anyChanges = FALSE; //Boolean variable which is changed to true if any field is filled out and profile information is updated 

        if (isset($_POST['new_full_name'])) { //Checks if new name is entered
            $updatedFullName = validate($_POST['new_full_name']);
            if (!empty($updatedFullName)) { //Validates new name and checks if name is less than 50 characters 
                if (strlen($updatedFullName) <= 50) {
                    $sql = "UPDATE ClientInformation SET full_name = '$updatedFullName' WHERE id={$_SESSION['id']}";
                    if ($conn->query($sql) === TRUE) { //Update name in database
        ...
        if (isset($_POST['new_address1'])) { //Checks if new address 1 is entered
            $updatedAddress1 = validate($_POST['new_address1']);
            if (!empty($updatedAddress1)) {
                if (strlen($updatedAddress1) <= 100) { //Validates new address 1 and checks if address is less than 100 characters 
                    $sql = "UPDATE ClientInformation SET address1 = '$updatedAddress1' WHERE id={$_SESSION['id']}";
                    if ($conn->query($sql) === TRUE) { //Update address 1 in database
        ...
        if (isset($_POST['new_address2'])) { //Checks if new address 2 is entered
            $updatedAddress2 = validate($_POST['new_address2']);
            if (!empty($updatedAddress2)) {
                if (strlen($updatedAddress2) <= 100) {  //Validates new address 2 and checks if address is less than 100 characters 
                    $sql = "UPDATE ClientInformation SET address2 = '$updatedAddress2' WHERE id={$_SESSION['id']}";
                    if ($conn->query($sql) === TRUE) { //Update address 2 in database
        ...
        if (isset($_POST['new_city'])) { //Checks if new city is entered
            $updatedCity = validate($_POST['new_city']);
            if (!empty($updatedCity)) {
                if (strlen($updatedCity) <= 100) { //Validates new city and checks if city is less than 100 characters 
                    $sql = "UPDATE ClientInformation SET city = '$updatedCity' WHERE id={$_SESSION['id']}";
                    if ($conn->query($sql) === TRUE) { //Update city in database
                        $updatedItems .= "City; ";
                        $anyChanges = TRUE;
                    } else {
                        header("Location: editProfile.php?error=Error updating record.");
        ...
        if (isset($_POST['new_state'])) { //Checks if new state is entered
            ...
            if (!empty($updatedState)) {
                ...
                if ($conn->query($sql) === TRUE) { //Update state in database
                    $updatedItems .= "State; ";
                    $anyChanges = TRUE;
                } else {
                    header("Location: editProfile.php?error=Error updating record.");
        ...
        if (isset($_POST['new_zipcode'])) { //Checks if new zipcode is entered
            $updatedZipcode = validate($_POST['new_zipcode']);
            if (!empty($updatedZipcode)) {
                if (!(strlen($updatedZipcode) < 5 || strlen($updatedZipcode) > 9)) { //Validates new zipcode and checks if zipcode is between 5 and characters 
                    $sql = "UPDATE ClientInformation SET zipcode = '$updatedZipcode' WHERE id={$_SESSION['id']}";
                    if ($conn->query($sql) === TRUE) { //Update zipcode in database
        ...
        if ($anyChanges === TRUE) { //If any fields were updated, update relevant session variables and reload page with message telling users what was updated
