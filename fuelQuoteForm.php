...
require_once('pricing_module.php'); 
    ...
    if (isset($_SESSION['name']) && isset($_SESSION['full_address']) && isset($_SESSION['id'])) {
        if (isset($_POST['gallons_requested']) && isset($_POST['delivery_date'])) {
            ...
            //Validate all entries to make sure data complies to our restrictions 
            if (empty($gallonsRequested)) {
                ...
                //fetch information from user profile to pass to pricing module to generate price
                $sql = "SELECT * FROM ClientInformation WHERE id={$_SESSION['id']}";
                $result2 = mysqli_query($conn, $sql);
                $row2 = mysqli_fetch_assoc($result2);
                $state = $row2['state'];

                $calculate = new pricing_module(); //create new pricing module and calculate price using location, gallons requested, and client history as factors
                ...
                //Insert client's order into the fuel database
